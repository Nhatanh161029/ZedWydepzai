...
local Window = OrionLib:MakeWindow({Name = "Test Hub", HidePremium = false, IntroText = "Test Hub Loader", SaveConfig = true, ConfigFolder = "TFA Hub"})
...
spawn(function()
    while task.wait() do 
        if af then
            pcall(function()
                if World2 == true and Beli >= 10000000 then
                    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Cousin","Buy")
                    toposition(CFrame.new(-380.47927856445, 77.220390319824, 255.82550048828))
                elseif World2 == true and Beli <= 10000000 then 
                        ...
                        if v.Name:find("Chest") or v.Name:find("Chest1") or v.Name:find("Chest2") or v.Name:find("Chest3") or v.Name:find("CursedTreasure") then 
                            if game:GetService("Workspace"):FindFirstChild(v.Name) then
                                if (v.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= math.huge then
                                    repeat task.wait()
                                    if game:GetService("Workspace"):FindFirstChild(v.Name) then
                                        delay(0,function()game:service("VirtualInputManager"):SendKeyEvent(true,"Q",false,game)
                                            game:service("VirtualInputManager"):SendKeyEvent(false,"Q",false,game)end)
